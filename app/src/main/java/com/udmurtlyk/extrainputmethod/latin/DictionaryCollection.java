...
public final class DictionaryCollection extends Dictionary {
    ...
    public ArrayList<SuggestedWords.SuggestedWordInfo> getSuggestions(final WordComposer composer,
        ...
        ArrayList<SuggestedWords.SuggestedWordInfo> suggestions = dictionaries.get(0).getSuggestions(composer,
                prevWordsInfo, proximityInfo, settingsValuesForSuggestion, sessionId,
                inOutLanguageWeight);
        if (null == suggestions) suggestions = new ArrayList<>();
        final int length = dictionaries.size();
        for (int i = 1; i < length; ++ i) {
            final ArrayList<SuggestedWords.SuggestedWordInfo> sugg = dictionaries.get(i).getSuggestions(composer,
                    prevWordsInfo, proximityInfo, settingsValuesForSuggestion, sessionId,
                    inOutLanguageWeight);
